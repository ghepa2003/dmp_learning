...
void QuaternionDMP::learnFromDemonstration(const std::vector<Sample>& demo) {
    ...
    // Locally weighted regression to compute the weights for the forcing term, independently per dimension.
    for (int d = 0; d < 3; ++d) {
        ...
        for (size_t k = 0; k < N; ++k) {
            ...
            double f_target = tau_ * eta_dot[k](d) - alpha_z_ * (2.0 * beta_z_ * log_err(d) - eta[k](d));
            double s = x_t[k];
