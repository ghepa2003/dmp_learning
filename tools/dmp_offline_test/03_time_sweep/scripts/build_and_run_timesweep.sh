...
if [ ${#demo_files[@]} -eq 0 ]; then
    echo "No demo_synth_*.csv files found in data/. Generate matrix first with: python3 03_time_sweep/scripts/generate_sweep_matrix.py --outdir data"
    exit 1
fi
...
python3 03_time_sweep/scripts/plot_sweep_study.py \
