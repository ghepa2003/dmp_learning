...
def generate(duration, dx, dy, dz, dt=0.001, transition_duration=10.0,
    ...
    Questo e' cio' che rende il confronto tra durate diverse significativo
    per lo studio di risoluzione delle basi: a parita' di transition_duration,
    una richiesta di `duration` maggiore comprime la stessa identica
    caratteristica di moto in una frazione di fase sempre piu' piccola,
    invece di dilatare l'intera forma (che sarebbe invariante per costruzione
    nella formulazione DMP, e quindi non mostrerebbe alcun effetto).
