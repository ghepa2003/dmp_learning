...
//
// Zero ROS dependencies on purpose - only Eigen. This is the "core" half of
// the core/wrapper split: it must stay usable in a plain executable or a
// unit test, without rclcpp anywhere in this file.
class DMP {
public:
    ...
    //   (critically damped when beta_z = alpha_z / 4, e.g. 25 / 6.25)
    ...
    // weighted regression (LWR). Sets internally: tau (= demo duration),
    ...
    // YAML), bypassing learnFromDemonstration. Used by dmp_io when loading.
